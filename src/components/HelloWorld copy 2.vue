 ...
 * @LastEditTime: 2023-03-31 22:04:06
...
onMounted(() => {
  ...
  const chart = L.control({ position: 'bottomright' })

  chart.onAdd = function () {
    const div = L.DomUtil.create('div', 'info chart')
    div.id = 'chatrdemo'
  ...
  chart.addTo(mapDom)
  const myChart = echart.init(document.getElementById('chatrdemo')!)
  const option = {
    tooltip: {
      trigger: 'axis'
    },
